...
for(var i = 0; i<pokemon.length;i++){
    if (pokemon[i].id %3 ===0)
    console.log("Iid divisible tonks "+pokemon[i].name)
}
for(var i = 0; i<pokemon.length;i++){
    if (pokemon[i].types.length >1){
    console.log("More than one tonks " +pokemon[i].name)
    }
}
for(var i = 0; i<pokemon.length;i++){
    if (pokemon[i].types[0]==="poison"){
        console.log("Poison tonks = " +pokemon[i].name)
        console.log("Poison tonk RVERSED "+pokemon[i].name)
        let pokeArray = "";
        for(var j = pokemon[i].name.length-1;j>0;j--)
        {
            pokeArray += pokemon[i].name[j];
...
for(var i = 0; i<pokemon.length;i++){
    if (pokemon[i].types[1]==="flying"){
        console.log("Flying Tonks "+pokemon[i].name)
    }
